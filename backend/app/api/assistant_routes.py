# this file handles assistant chat messages and tool calls to the model
...
MAX_TOOL_ROUNDS = 6
MAX_TOKENS = 4096
MEDGUARD_TOOL = {'name': 'medguard_analyze_pair', 'description': 'Run the MedGuard Bio_ClinicalBERT pipeline on two explicit drug names: NER on the inference text, DDI interaction type, severity, KG context, and Lipinski features. Use this whenever the user asks about interactions, safety of combining two medications, or similar. Pass normalized generic names when possible (e.g. warfarin, ibuprofen). Optional clinical_context is a short user sentence passed through to the model.', 'input_schema': {'type': 'object', 'properties': {'drug_a': {'type': 'string', 'description': 'First drug name (ingredient preferred).'}, 'drug_b': {'type': 'string', 'description': 'Second drug name (ingredient preferred).'}, 'clinical_context': {'type': 'string', 'description': 'Optional short clinical sentence for context.'}}, 'required': ['drug_a', 'drug_b']}}
...
# this class groups logic for AssistantMessage
class AssistantMessage(BaseModel):
    role: Literal['user', 'assistant']
    content: str

# this class groups logic for AssistantChatRequest
class AssistantChatRequest(BaseModel):
    messages: List[AssistantMessage] = Field(..., min_length=1, description='Conversation history ending with the latest user message.')

# this class groups logic for ToolTraceItem
...
# this class groups logic for AssistantChatResponse
...
# this function is used to handle ddi to tool json
...
# this function is used to handle execute tool
...
# this function is used to handle anthropic client
def _anthropic_client():
    try:
        import anthropic
    except ImportError as exc:
        raise HTTPException(status_code=503, detail='Anthropic SDK not installed. Run: pip install -r requirements-llm.txt') from exc
    key = os.environ.get('ANTHROPIC_API_KEY', '').strip()
    if not key:
        raise HTTPException(status_code=503, detail='ANTHROPIC_API_KEY is not set. Add it to the environment for the assistant endpoint.')
    return anthropic.Anthropic(api_key=key)

@router.get('/status')
# this function is used to handle assistant status
def assistant_status():
    ...
    try:
        ...
        sdk_ok = False
    return {'anthropic_sdk_installed': sdk_ok, 'anthropic_api_key_configured': key_ok, 'default_model': DEFAULT_ANTHROPIC_MODEL}

@router.post('/chat', response_model=AssistantChatResponse)
# this async function is used to handle assistant chat
async def assistant_chat(request: AssistantChatRequest):
    client = _anthropic_client()
    model = os.environ.get('ANTHROPIC_MODEL', DEFAULT_ANTHROPIC_MODEL).strip() or DEFAULT_ANTHROPIC_MODEL
    messages: List[Dict[str, Any]] = [{'role': m.role, 'content': m.content} for m in request.messages]
    tool_trace: List[ToolTraceItem] = []
    stop_reason: Optional[str] = None
    final_text_parts: List[str] = []
    last_response: Any = None

    # this function is used to handle one turn
    def _one_turn(msgs: List[Dict[str, Any]]):
        return client.messages.create(model=model, max_tokens=MAX_TOKENS, system=SYSTEM_PROMPT, tools=[MEDGUARD_TOOL], messages=msgs)
    for _ in range(MAX_TOOL_ROUNDS):
        ...
        for block in tool_use_blocks:
            ...
            if not isinstance(raw_input, dict):
                raw_input = {}
            out_str, ok, err = await asyncio.to_thread(_execute_tool, block.name, raw_input)
            tool_trace.append(ToolTraceItem(tool=block.name, input=dict(raw_input), ok=ok, error=err))
            tool_result_payloads.append({'type': 'tool_result', 'tool_use_id': block.id, 'content': out_str})
    ...
    if not reply and last_response is not None:
        ...
        reply = reply.strip()
    if not reply:
        reply = 'No assistant text was returned. If a tool ran, check tool_trace in the JSON response for model outputs.'
    return AssistantChatResponse(reply=reply, model=model, tool_trace=tool_trace, stop_reason=stop_reason)
